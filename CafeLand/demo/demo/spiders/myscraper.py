...
class MyscraperSpider(scrapy.Spider):
    ...
    def parse_details(self, response):
        ...
        yield {
            ...
            'Diện tích: ': area.strip() if area else '',
            'Ngày đăng': date.strip() if date else '',
            'Tổng vốn đầu tư': total_investment.strip() if total_investment else '',
            'Xếp hạng': average_rating.strip() if average_rating else '',
            'Mô tả': moTa.strip() if moTa else '',
        }
